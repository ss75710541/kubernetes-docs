...
alertmanager:
  ...
  config:
    ...
    - name: critical
      email_configs:
      - to: 'liujinye@hisuntech.com,pang_zheng@hisuntech.com'
      wechat_configs:
      - send_resolved: true
        to_user: '@all'
        agent_id: 'xxxxxxxx'
...
### 解决kube-controller-manager metrics 获取异常问题

修改master 主机 /etc/kubernetes/manifests/kube-controller-manager.yaml
...
### 解决 kubeScheduler metrics 获取异常问题
